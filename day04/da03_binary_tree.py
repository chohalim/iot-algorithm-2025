...
if __name__ == '__main__':
    ...
    for name in nameAry[1:]:
        ...
        current = root
        while True:
            if name < current.data: # (현재 name) < (노드 안 데이터)라면,
    ...
    ## 트리노드 삭제 # 마마무, 에이핑크 삭제(레드벨벳 삭제하면 다 깨짐)
    ...
    while True: # 삭제할 때
        if delName == current.data:
            ...
            if current.left == None and current.right == None: # 리프노드
                ...
                if parent.left == current:
                    parent.left = current.left # 부모랑 손자랑 연결해라
                else:
                    parent.right = current.left
                del(current)

            elif current.left == None and current.right != None: # 오른쪽 노드에 자식있고, 왼쪽에 없을 때
                if parent.left == current:
