...
# 최소 비용신장 트리
from operator import itemgetter # 튜플, 리스트의 아이템을 인덱스해당 아이템 가져올 때 쓰는 모듈
...
def findVertex(g, findVtx):
    ...
    while len(stack) != 0: 
        ...
        for vertex in range(SIZE):
            if g.graph[current][vertex] != 0: # 가중치로 할 것이라서 == 1로 안함
...
while len(newAry) > (SIZE-1): # 간선의 수 = (정점의 수 - 1) 될 때까지 반복
